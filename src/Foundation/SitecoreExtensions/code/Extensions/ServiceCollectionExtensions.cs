namespace Sug.Foundation.SitecoreExtensions.Extensions
{
	...
	public static class ServiceCollectionExtensions
	{
		...
		public static void AddWebApiControllers(this IServiceCollection serviceCollection, params string[] assemblyFilters)
		{
			var assemblyNames = new HashSet<string>(assemblyFilters.Where(filter => !filter.Contains('*')));
			var wildcardNames = assemblyFilters.Where(filter => filter.Contains('*')).ToArray();
			...
			AddWebApiControllers(serviceCollection, assemblies);
		}

		public static void AddWebApiControllers(this IServiceCollection serviceCollection, params Assembly[] assemblies)
		{
			var controllers = GetTypesImplementing<ApiController>(assemblies)
